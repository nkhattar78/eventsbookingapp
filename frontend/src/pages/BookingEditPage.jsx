...
export default function BookingEditPage() {
  ...
  return (
    <Box p={3} maxWidth={500} mx="auto">
      <Paper sx={{ p: 3 }}>
        <Typography variant="h5" mb={2}>Edit Booking</Typography>
        {error && <Alert severity="error" sx={{ mb:2 }}>{error}</Alert>}
        <Box component="form" onSubmit={handleSubmit} noValidate>
          <Stack spacing={2}>
            <TextField label="Customer Name" name="customer_name" value={form.customer_name} onChange={handleChange} required />
            <TextField label="Customer Email" name="customer_email" value={form.customer_email} onChange={handleChange} type="email" required />
